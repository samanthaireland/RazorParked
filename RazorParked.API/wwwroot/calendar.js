// CALENDAR JS -- RazorParked v4
// Single month, game filter bar, same-day depart fix
...
function _injectStyles() {
    ...
    s.textContent =
        ...
        // Game filter bar -- scrollable horizontal strip
        '.rpc-gf{display:flex;gap:5px;padding:8px 12px;overflow-x:auto;border-bottom:1px solid #e5e0d8;background:#faf9f7;flex-wrap:nowrap;-webkit-overflow-scrolling:touch}' +
        '.rpc-gf::-webkit-scrollbar{height:3px}' +
        '.rpc-gf::-webkit-scrollbar-thumb{background:#ddd;border-radius:2px}' +
        '.rpc-gfb{border:1px solid #e5e0d8;border-radius:20px;padding:3px 10px;font-size:11px;font-weight:500;background:#fff;cursor:pointer;font-family:inherit;white-space:nowrap;transition:all .12s;flex-shrink:0}' +
        '.rpc-gfb:hover{border-color:#9d2235;color:#9d2235}' +
        '.rpc-gfb.fb-h{border-color:#d4a0a8;background:#fdf0f2;color:#7a1a28}' +
        '.rpc-gfb.fb-a{border-color:#b5c9e8;background:#eef4fb;color:#1a3a6e}' +
        '.rpc-gfb.bb-h{border-color:#a8d5b8;background:#edf7f0;color:#1a4a2e}' +
        '.rpc-gfb.bb-a{border-color:#c5dac9;background:#f0f7f2;color:#2d5a3a}' +
        '.rpc-gfb.rpc-gfsel{background:#222;color:#fff;border-color:#222}' +
        // Body: single month left + right panel
        '.rpc-body{display:grid;grid-template-columns:1fr 290px}' +
        '.rpc-left{padding:14px;border-right:1px solid #e5e0d8}' +
        // Single month header
        '.rpc-mhdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}' +
        '.rpc-mttl{font-size:13px;font-weight:600;color:#1a1208}' +
        '.rpc-mnav{display:flex;gap:4px}' +
        '.rpc-nb{width:26px;height:26px;border-radius:50%;border:1px solid #e5e0d8;background:none;cursor:pointer;font-size:13px;color:#1a1208;display:flex;align-items:center;justify-content:center;padding:0}' +
        '.rpc-nb:hover{background:#f0f0f0}' +
        '.rpc-dgrid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px}' +
        '.rpc-dow{font-size:10px;color:#bbb;text-align:center;padding:3px 0;font-weight:500}' +
        '.rpc-day{position:relative;aspect-ratio:1;display:flex;align-items:center;justify-content:center;font-size:12px;cursor:pointer;color:#1a1208;flex-direction:column;transition:background .1s;border-radius:50%}' +
        ...
        '.rpc-legend{display:flex;gap:8px;margin-top:10px;flex-wrap:wrap}' +
        ...
        '.rpc-right-inner{overflow-y:auto;max-height:320px;min-height:0}' +
        // Game list
        '.rpc-gl-month{font-size:10px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:#bbb;padding:6px 12px 3px;background:#faf9f7;border-bottom:1px solid #f0f0f0;position:sticky;top:0;z-index:1}' +
        ...
        '.rpc-gl-date{font-size:12px;font-weight:700;color:#888;min-width:28px;text-align:center;line-height:1.2}' +
        '.rpc-gl-date span{display:block;font-size:9px;font-weight:500;color:#bbb;text-transform:uppercase}' +
        '.rpc-gl-bar{width:3px;border-radius:2px;align-self:stretch;flex-shrink:0;min-height:26px}' +
        ...
        '.rpc-game-banner{padding:9px 12px;border-bottom:1px solid #e5e0d8}' +
...
var RazorParkedCalendar = (function () {
    ...
    Cal.prototype._shell = function () {
        ...
        return '<div class="rpc">'
            ...
            + '</div>'
            + '<div class="rpc-gf" id="rpc-gf-' + i + '"></div>'
            + '<div class="rpc-body">'
            + '<div class="rpc-left">'
            + '<div class="rpc-mhdr">'
            + '<span class="rpc-mttl" id="rpc-mttl-' + i + '"></span>'
            + '<div class="rpc-mnav">'
            + '<button class="rpc-nb" onclick="' + ref + '._shift(-1)">&#8249;</button>'
            + '<button class="rpc-nb" onclick="' + ref + '._shift(1)">&#8250;</button>'
            + '</div></div>'
            + '<div id="rpc-month-' + i + '"></div>'
    ...
    Cal.prototype._renderGameFilter = function () {
        ...
        var ref = "window['_rpc_" + i + "']";
        var html = '<button class="rpc-gfb rpc-gfsel" onclick="' + ref + '._clearFilter(this)">All dates</button>';

        if (sport === 'all' || sport === 'fb') {
            Object.keys(FB_SCHEDULE).sort().forEach(function (k) {
                var g = FB_SCHEDULE[k], d = new Date(k + 'T12:00:00');
                var lbl = (d.getMonth() + 1) + '/' + d.getDate() + ' ' + (g.home ? 'vs ' : '@ ') + g.opp;
                var cls = 'rpc-gfb ' + (g.home ? 'fb-h' : 'fb-a');
                html += '<button class="' + cls + '" data-key="' + k + '" onclick="' + ref + '._jumpToGame(\'' + k + '\',this)">' + lbl + '</button>';
            });
        }
        if (sport === 'all' || sport === 'bb') {
            var seen = {};
            Object.keys(BB_SCHEDULE).sort().forEach(function (k) {
                var g = BB_SCHEDULE[k], sk = g.opp + (g.home ? 'H' : 'A');
                if (seen[sk]) return; seen[sk] = 1;
                var d = new Date(k + 'T12:00:00');
                var lbl = (d.getMonth() + 1) + '/' + d.getDate() + ' ' + (g.home ? 'vs ' : '@ ') + g.opp;
                var cls = 'rpc-gfb ' + (g.home ? 'bb-h' : 'bb-a');
                html += '<button class="' + cls + '" data-key="' + k + '" onclick="' + ref + '._jumpToGame(\'' + k + '\',this)">' + lbl + '</button>';
            });
        }
        div.innerHTML = html;
    ...
    Cal.prototype._clearFilter = function (btn) {
        ...
        var t = new Date(); this.bYear = t.getFullYear(); this.bMonth = t.getMonth();
        this._renderCal();
    };

    Cal.prototype._renderCal = function () {
        var ttl = document.getElementById('rpc-mttl-' + this.id);
        var el = document.getElementById('rpc-month-' + this.id);
        if (!ttl || !el) return;
        ttl.textContent = MONTH_NAMES[this.bMonth] + ' ' + this.bYear;
        el.innerHTML = this._month(this.bYear, this.bMonth);
    };
    ...
    Cal.prototype._month = function (y, m) {
        ...
        for (var d = 1; d <= total; d++) {
            ...
            if (isA) cls += ' rarr';
            if (isD && !isA) cls += ' rdep';
            if ((isA && !this.departKey) || (isA && isD)) cls += ' rsolo';
            if (inR) cls += ' rin';
            var dots = '';
            ...
            html += '<div class="' + cls + '"' + (tip ? ' title="' + tip + '"' : '') + ' onclick="' + ref + '._click(\'' + k + '\')">'
                + '<div style="font-size:12px;line-height:1">' + d + '</div>'
    ...
    Cal.prototype._shift = function (dir) {
        this.bMonth += dir;
        if (this.bMonth > 11) { this.bMonth = 0; this.bYear++; }
        if (this.bMonth < 0) { this.bMonth = 11; this.bYear--; }
        this._renderCal();
    };
    ...
    Cal.prototype._showDayDetail = function () {
        ...
        var sdHint = isSameDay ? '<div class="rpc-sameday-hint">Same-day: pick arrival then departure time</div>' : '';
        if (fb) {
            var isH = fb.home;
    ...
    Cal.prototype._renderSlots = function () {
        ...
        var isSameDay = this.arriveKey === this.departKey;
        var i = this.id, ref = "window['_rpc_" + i + "']";
        ...
        for (var x = 0; x < slots.length; x++) {
            ...
            if (isND && !crossedMid) {
                crossedMid = true;
                html += '</div><div class="rpc-nd-div">' + nextLabel + ' (+1 day)</div><div class="rpc-sgrid">';
            }
            ...
            var status = isB ? 'Booked' : isSel ? (isArr ? 'Arrive' : 'Depart') : isFbG ? 'Kickoff' : isBbG ? '1st pitch' : isND ? '+1 day' : 'Open';
            html += '<button class="' + cls + '"' + (isB ? ' disabled' : '') + ' onclick="' + ref + '._pick(' + x + ',\'' + s + '\',' + isArr + ')">'
                + tag + '<div class="rpc-slot-t">' + s + '</div>'
                + '<div class="rpc-slot-s">' + status + '</div></button>';
        }
        html += '</div>';
    ...
    Cal.prototype._updateBBoxes = function () {
        ...
        if (dur) {
            ...
            else if (!this.departKey) { dur.textContent = 'Click depart date — same date OK for same-day booking'; }
            else if (this.arriveTime && this.departTime) {
                var nights = Math.round((_ms(this.departKey) - _ms(this.arriveKey)) / 86400000);
                var ns = isSameDay ? '<strong>Same day</strong>' : '<strong>' + nights + ' night' + (nights !== 1 ? 's' : '') + '</strong>';
                dur.innerHTML = ns + ' &bull; ' + _short(this.arriveKey) + ' ' + this.arriveTime + ' &rarr; ' + _short(this.departKey) + ' ' + this.departTime;
            } else if (this.arriveTime) {
                dur.textContent = 'Now pick departure time';
            } else {
                var nights2 = Math.round((_ms(this.departKey) - _ms(this.arriveKey)) / 86400000);
                var ns2 = isSameDay ? 'Same day' : nights2 + ' night' + (nights2 !== 1 ? 's' : '');
                dur.innerHTML = '<strong>' + ns2 + '</strong> &bull; ' + _short(this.arriveKey) + ' &rarr; ' + _short(this.departKey) + ' &bull; Pick arrival time';
            }
        }
    };
    ...
    Cal.prototype._confirm = function () {
        ...
        var sum = document.getElementById('rpc-sum-' + this.id);
        if (sum) sum.textContent = _short(this.arriveKey) + ' ' + this.arriveTime + ' → ' + _short(this.departKey) + ' ' + this.departTime;
    };
    ...
    Cal.prototype._renderGameList = function () {
        var gl = document.getElementById('rpc-gamelist-' + this.id);
        if (!gl) return;
        var sport = this.sport, i = this.id, ref = "window['_rpc_" + i + "']";
        var games = [];
        if (sport === 'all' || sport === 'fb') {
        ...
        games.forEach(function (item) {
            ...
            html += '<div class="rpc-gl-item" onclick="' + ref + '._jumpToGame(\'' + item.k + '\',null)">'
